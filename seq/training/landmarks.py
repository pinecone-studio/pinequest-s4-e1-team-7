...
def pack_raw_vector(pose_res, hand_res) -> tuple[np.ndarray, bool]:
    ...
    # Hands assigned by handedness label for consistency with the web client.
...
def normalize(raw: np.ndarray) -> np.ndarray:
    ...
    for p in range(n_points):
        xi = p * C.N_COORDS
        yi = xi + 1
        # Leave zero-padded (absent) points at zero.
    ...
    # Presence flags untouched (indices LEFT_PRESENT_IDX, RIGHT_PRESENT_IDX).
