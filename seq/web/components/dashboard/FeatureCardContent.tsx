  ...
  return (
    <>
      ...
      <div className="relative z-10 flex h-full flex-col justify-between p-7 md:p-8">
        <div>
          <h3 className="text-[24px] font-bold leading-snug md:text-[28px] lg:text-[32px]"
            style={{ color: f.dark ? "#eaf0f8" : "var(--text)" }}>
            {f.title}
          </h3>
          <p className="mt-2 text-[14px] leading-relaxed md:text-[15px] lg:text-[16px]"
            style={{ color: f.dark ? "rgba(234,240,248,0.75)" : "var(--text-3)" }}>
            {f.sub}
          </p>
        </div>
        <div className="inline-flex items-center gap-2 self-start rounded-full px-5 py-2.5 text-[14px] font-bold transition-all duration-200 group-hover:brightness-110 group-hover:scale-105"
          style={{ background: f.dark ? "rgba(0,0,0,0.35)" : "var(--olive)", color: f.dark ? "#eaf0f8" : "#0d1e35" }}>
          Эхлэх
          <div className="flex h-6 w-6 items-center justify-center rounded-full"
            style={{ background: f.dark ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.12)" }}>
