...
export function CallCaptionHistory({
  ...
  if (variant === "mobile") {
    return (
      <div className="flex h-full min-h-0 flex-col" aria-label="Ярианы түүх">
        <button
          type="button"
          onClick={onToggle}
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside
        className="mt-5 pointer-events-none absolute bottom-[calc(5.5rem+env(safe-area-inset-bottom))] right-0 top-[calc(env(safe-area-inset-top)+3.5rem)] z-30 hidden min-w-[220px] transition-transform duration-300 ease-out md:flex md:flex-col"
        style={{
          width: SIDEBAR_W,
      ...
      >
        <div
          ...
          style={{
            ...
            border:
              "1px solid color-mix(in srgb, var(--border-c) 50%, transparent)",
          }}
        >
          {/* Header */}
          <div
          ...
          <div
            ref={scrollRef}
            className="flex flex-1 flex-col justify-end overflow-y-auto px-3 pb-3 pt-2"
          >
            {entries.length === 0 ? (
              <p
