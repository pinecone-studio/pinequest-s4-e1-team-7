    ...
    <div className="absolute left-0 top-0 flex h-full flex-col justify-center bg-black px-[5vw]"
      ...
      {compare === "before" && (
        <div className="mt-8 pl-[76px] flex flex-col gap-1">
          <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: "rgba(255,255,255,0.35)" }}>Эх сурвалж</p>
          {SOURCES.map(s => (
            <p key={s} className="text-[11px] leading-snug" style={{ color: "rgba(255,255,255,0.22)" }}>{s}</p>
          ))}
        </div>
      )}
    </div>
