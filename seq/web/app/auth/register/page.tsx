      ...
      <div className="relative z-10 flex w-full flex-col border-l lg:w-7/12 xl:w-1/2" style={{ background: "var(--surface)", borderColor: "var(--border-c)" }}>
        <div className="flex items-center justify-between border-b px-12 pt-12 pb-5" style={{ borderColor: "var(--border-c)" }}>
          <div className="flex items-center gap-1">
            ...
            <Link href="/auth/login" className="rounded-full px-5 py-2 text-sm font-medium transition-colors"
        ...
        <div className="flex flex-1 items-center justify-center overflow-y-auto px-12 py-8">
          <div className="w-full max-w-md">
            <div className="mb-8">
              <h2 className="text-3xl font-black tracking-tight" style={{ color: "var(--text)" }}>Бүртгэл үүсгэх</h2>
              <p className="mt-2 text-sm" style={{ color: "var(--text-3)" }}>Чат болон дуудлагад ашиглагдах мэдээллээ оруулна уу.</p>
            </div>

            <form onSubmit={submit} className="space-y-4">
              {[
                { id: "name", label: "Нэр", type: "text", value: name, set: setName, Icon: User, placeholder: "Бат-Эрдэнэ" },
                { id: "email", label: "И-мэйл хаяг", type: "email", value: email, set: setEmail, Icon: Mail, placeholder: "example@domain.com" },
                { id: "phone", label: "Утасны дугаар", type: "text", value: phone, set: setPhone, Icon: Phone, placeholder: "99112233" },
              ].map(({ id, label, type, value, set, Icon, placeholder }) => (
                <div key={id}>
                  <label htmlFor={id} className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color: "var(--text-2)" }}>{label}</label>
                  <div className="relative">
                    <Icon className="absolute left-4 top-1/2 -translate-y-1/2 size-4" style={{ color: "var(--text-3)" }} />
                    <input id={id} type={type} value={value} onChange={e => set(e.target.value)} placeholder={placeholder} required
              ...
              {[
                { id: "password", label: "Нууц үг", value: password, set: setPassword, show: showPassword, toggle: () => setShowPassword(v => !v) },
                { id: "confirm", label: "Нууц үг баталгаажуулах", value: confirmPassword, set: setConfirmPassword, show: showConfirm, toggle: () => setShowConfirm(v => !v) },
              ].map(({ id, label, value, set, show, toggle }) => (
                <div key={id}>
                  <label htmlFor={id} className="mb-1.5 block text-xs font-bold uppercase tracking-wider" style={{ color: "var(--text-2)" }}>{label}</label>
                  <div className="relative">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 size-4" style={{ color: "var(--text-3)" }} />
                    <input id={id} type={show ? "text" : "password"} value={value} onChange={e => set(e.target.value)} placeholder="••••••••" required
                      className="w-full rounded-xl border py-3.5 pl-11 pr-12 text-sm outline-none transition-all"
                      style={{ background: "var(--surface-2)", borderColor: "var(--border-c)", color: "var(--text)" }} />
                    <button type="button" onClick={toggle} className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5" style={{ color: "var(--text-3)" }}>
                      {show ? <Eye size={16} /> : <EyeClosed size={16} />}
                    </button>
                  </div>
                  {id === "confirm" && confirmPassword && password !== confirmPassword && (
                    <p className="mt-1 text-xs text-red-500">Нууц үгүүд таарахгүй байна.</p>
                  )}
                </div>
              ))}
            ...
            <p className="mt-6 text-center text-sm" style={{ color: "var(--text-3)" }}>
              Та бүртгэлтэй бол{" "}
              <Link href="/auth/login" className="font-bold hover:underline" style={{ color: "var(--olive)" }}>Нэвтрэх</Link>
            </p>
          </div>
        </div>
      </div>
